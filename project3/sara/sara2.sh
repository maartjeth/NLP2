...
input_archive_fn="$project_root/dev/dev-$name.gz"

# Prepare scratch
echo Copying files...
mkdir -p "$scratch/output-$name"
...
echo Zipping...
echo "to $scratch/output-$name"

cd "$scratch/output-$name"
...
cp -r "$scratch/output-$name" "$project_root"
echo "Done."
